...
def scan2D(windowMinX=0.0, windowMaxX=1., windowMinY=0.0, windowMaxY=1.0,
    ...
    histo2D_signed_significance=ROOT.TH2D("histo2D_signed_significance","histo2D_signed_significance",nBinsX, scanMinX, 1.0, nBinsY, scanMinY, 1.0 )
    histo2D_signed_significance.Sumw2()
    ...
    for xCut in numpy.arange(scanMinX, scanMaxX, 0.01):
        for yCut in numpy.arange(scanMinY, scanMaxY, 0.01):
            ...
            if verbosity>0:
                ...
                print("")
            # calculate stuff
            closure, closure_error, significance, signed_significance = 0,0,0,0
            if dataMode=="data":
                if verbosity>0:
                    print("ABCD in these regions")
                    obs_A, obs_error_A = theABCD.getIntegralAndErrorInRegion("A", "data")
                ...
                signed_significance=theABCD.getSimpleClosureSignificance(dataMode=dataMode, verbosity=verbosity, doUnsigned=False)
            ...
            histo2D_signed_significance.SetBinContent(binX, binY, signed_significance)
    ...
    return histo2D_significance, histo2D_closure, histo2D_closure_error, histo2D_maxAllowedNonClosure, histo2D_signed_significance

def produce1DDistribution(the2Dhisto=None, minX=0, minY=0):
    binContentList=[]
    nBinsX=the2Dhisto.GetNbinsX()+2
    nBinsY=the2Dhisto.GetNbinsY()+2
    for iBinX in range(nBinsX):
        if the2Dhisto.GetXaxis().GetBinLowEdge(iBinX)<minX:
            continue
        for iBinY in range(nBinsY):
            if the2Dhisto.GetYaxis().GetBinLowEdge(iBinY)<minY:
                continue
            binContentList.append(the2Dhisto.GetBinContent(iBinX, iBinY))
    minValue=min(binContentList)        
    maxValue=max(binContentList)
    the1Dhisto=ROOT.TH1D("the1Dhisto","the1Dhisto",100,minValue-1, maxValue+1)
    for value in binContentList:
        if value==0.0:
            continue
        the1Dhisto.Fill(value)
    return the1Dhisto

def plot1DDistribution(the2Dhisto=None, minX=0, minY=0, title="", axisTitle="", outputpath="", doFit=False):
    ROOT.gStyle.SetOptStat(1)
    the1Dhisto=produce1DDistribution(the2Dhisto, minX, minY)
    the1Dhisto.SetTitle(title)
    the1Dhisto.GetXaxis().SetTitle(axisTitle)
    c=ROOT.TCanvas(title,title,1024,768)        
    the1Dhisto.Draw("histoE")
    # c.SetLogy()
    c.SaveAs(outputpath+".pdf")
    c.SaveAs(outputpath+".png")
    ROOT.gStyle.SetOptStat(0)
...
# create toplevel dir
toplevelDirName="closures_allRegions_jetComb12_V8"
# toplevelDirName="closures_allRegions_jetComb02_V7"
        ...
        for lowerbound in lowerbound_list:
            ...
            for theRegion, theHistoString, theDataMode in zip(region_list, histoString_list, dataMode_list):
                ...
                if theRegion=="VR":
                    ...
                    verbosity=0        
                histo2D_significance, histo2D_closure, histo2D_closure_error, histo2D_maxAllowedNonClosure, histo2D_signed_significance = scan2D(windowMinX=windowMinX, windowMaxX=windowMaxX, windowMinY=windowMinY, windowMaxY=windowMaxY,
                ...
                histo_list=[histo2D_significance, histo2D_closure, histo2D_closure_error, histo2D_maxAllowedNonClosure, histo2D_signed_significance]
                fom_list=["significance","closure","closure_error", "max_allowed_nonClosure", "signed_significance"]
                # blind
                if theRegion=="SR":
                ...
                for theHisto, theFom in zip(histo_list, fom_list):
                    ...
                    if theFom=="signed_significance":
                        theHisto.GetZaxis().SetTitle("significance = #frac{pred-obs}{total uncertainty}")
                        theHisto.GetZaxis().SetRangeUser(-5,5)
                ...
                if theRegion!="SR":
                    plot1DDistribution(the2Dhisto=histo2D_signed_significance, minX=0, minY=0, title="1D_significance_"+WP_tagName+"_"+theRegion+"_"+theDataMode, axisTitle="non-closure significance", outputpath=outputDir_level2+theRegion+"_"+theDataMode+"_"+windowString+"_"+"unrestricted_"+"1Dsignificance")
            ...
            for theRegion, theHistoString, theDataMode in zip(region_list, histoString_list, dataMode_list):
                ...
                # plot 1D distribution
                plot1DDistribution(the2Dhisto=histo2D_signed_significance, minX=0, minY=0, title="1D_significance_"+WP_tagName+"_"+theRegion+"_"+theDataMode, axisTitle="non-closure significance", outputpath=outputDir_level2+theRegion+"_"+theDataMode+"_"+windowString+"_"+"Xrestricted_"+"1Dsignificance")
            ...
            for theRegion, theHistoString, theDataMode in zip(region_list, histoString_list, dataMode_list):
                ...
                # plot 1D distribution
                plot1DDistribution(the2Dhisto=histo2D_signed_significance, minX=0, minY=0, title="1D_significance_"+WP_tagName+"_"+theRegion+"_"+theDataMode, axisTitle="non-closure significance", outputpath=outputDir_level2+theRegion+"_"+theDataMode+"_"+windowString+"_"+"Yrestricted_"+"1Dsignificance")
