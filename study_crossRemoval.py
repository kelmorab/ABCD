...
def scan2D(windowMinX=0.0, windowMaxX=1., windowMinY=0.0, windowMaxY=1.0,
            ...
            dataMode="data", verbosity=1, doRemoveCross=False):
    stepsize=0.01        

    theABCD=None
    if dataMode=="data":
        histo_data_obs=histoFile.Get(histoString.replace("$SAMPLE","data_obs")).Clone()
        histo_SM_Higgs = histoFile.Get(histoString.replace("$SAMPLE","SM_Higgs")).Clone()
        histo_TTbar = histoFile.Get(histoString.replace("$SAMPLE","TTbar")).Clone()
        histo_Others = histoFile.Get(histoString.replace("$SAMPLE","Others")).Clone()
        ...
        histo_QCD=histoFile.Get(histoString.replace("$SAMPLE","QCD")).Clone()
        theABCD=ABCD()
    ...
    if doRemoveCross:
        theABCD.printHistos("crosscheck_histos/original_histo_"+histoString.replace("$SAMPLE",""))
        theABCD.emptyBinsCrossWise(xValue=0.76, yValue=0.76)
        theABCD.printHistos("crosscheck_histos/modified_histo_"+histoString.replace("$SAMPLE",""))
...
toplevelDirName="closures_allRegions_jetComb12_removeCross_V7"
        ...
        for lowerbound in lowerbound_list:
            ...
            for theRegion, theHistoString, theDataMode in zip(region_list, histoString_list, dataMode_list):
                ...
                histo2D_significance, histo2D_closure, histo2D_closure_error, histo2D_maxAllowedNonClosure = scan2D(windowMinX=windowMinX, windowMaxX=windowMaxX, windowMinY=windowMinY, windowMaxY=windowMaxY,
                                                                                    ...
                                                                                    dataMode=theDataMode, verbosity=verbosity, doRemoveCross=True)
            ...
            #                                                                         scanMinX=scanMinX, scanMaxX=scanMaxX, scanMinY=scanMinY, scanMaxY=scanMaxY,
            #                                                                         histoFile=histoFile, histoString=theHistoString,
            #                                                                         dataMode=theDataMode, verbosity=0,doRemoveCross=True )
            #     histo_list=[histo2D_significance, histo2D_closure, histo2D_closure_error, histo2D_maxAllowedNonClosure]
            #     fom_list=["significance","closure","closure_error", "max_allowed_nonClosure"]
            #     for theHisto, theFom in zip(histo_list, fom_list):
            #         canvas=ROOT.TCanvas("canvas_"+WP_tagName+"_"+theRegion+"_"+theDataMode,"canvas_"+WP_tagName+"_"+theRegion+"_"+theDataMode,1024,768)
